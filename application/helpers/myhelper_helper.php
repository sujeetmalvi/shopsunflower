...
//   $point = round($number - $no , 2) * 100;
  ...
  // return $result;
     ...
     function save_activity_details($activitydesc) {
        ...
        $insert = $CI->db->insert('ActivityLogMaster', $new_activity_data);
    ...
    function sendEmail($RecipientEmailAddress, $EmailSubject, $EmailMessage, $Attachments='') {
        ...
        $mail->Username = "test@gmail.com";           // User email address
        $mail->Password = "abcedf";
        $mail->IsHTML(true); // Password in GMail

        $mail->SetFrom('info@test.com', 'tes1');     //Who is sending the email
        $mail->AddReplyTo("support@test.com', 'test2");  //Email address that receives the response
...
//insert_batch
    ...
    $insert = $CI->db->insert_batch('enquiries',$insert_data);

    if($insert){
        return $i ." recoreds imported successfuly";
    }else{
            ...
            if ((($_FILES[$imgvariable]["type"] == "image/png") || ($_FILES[$imgvariable]["type"] == "image/jpg") || ($_FILES[$imgvariable]["type"] == "image/jpeg") || ($_FILES[$imgvariable]["type"] == "image/gif") || ($_FILES[$imgvariable]["type"] == "application/vnd.ms-powerpoint") || ($_FILES[$imgvariable]["type"] == "application/pdf")
                ...
                if ($_FILES[$imgvariable]["error"] > 0) {
                    ...
                    if (file_exists("uploads/".$foldername."/". $_FILES[$imgvariable]["name"])) {
                        ...
                        if(file_exists("uploads/".$foldername."/". $filename)){
...
function get_comment_data_array($str){
    $t = explode(',',$str);
    $arr = array();
    foreach($t as $v){
    	$a = explode(':',$v);
