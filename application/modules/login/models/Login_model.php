...
class Login_model extends CI_Model {
   ...
   public function check_login() {
        ...
        $sql = "SELECT * FROM AdminUser WHERE `MobileNo`='".$loginname."' AND `LoginPassword` = AES_ENCRYPT('".LOGIN_SALT."','".$loginpassword."') ";
            ...
            if ($query) {
                if ($query->num_rows() > 0) {
                    ...
                    $data = array(
                        'user_id' => trim($datas[0]['iUserId']),
                        'UserId' => trim($datas[0]['iUserId']),
                        'user_role' => 'admin',
                        'user_roleid' => '0',
                        'UserRoleId' => '0',
                        'user_name' => trim($datas[0]['FullName'])
                        );
