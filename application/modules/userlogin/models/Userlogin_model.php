...
class Userlogin_model extends CI_Model {
   ...
   public function check_login() {
        ...
        $sql = "SELECT * FROM UserMaster WHERE `EmailId`='".$loginname."' AND `Password` = AES_ENCRYPT('".LOGIN_SALT."','".$loginpassword."') ";
            ...
            if ($query) {
                if ($query->num_rows() > 0) {
                    ...
                    $login_type = $_SERVER['LOGINROLES'][$datas[0]['LoginType']];

                    if($login_type== 'retailer'){
                        $loginsql = "SELECT * FROM retailer_master WHERE id='".$datas[0]['id']."' ";
                        $login_query = $this->db->query($loginsql);
                        $login_data = $login_query->result_array();

                        $data = array(
                        'user_id' => trim($datas[0]['id']),
                        'user_role' => $_SERVER['LOGINROLES'][$datas[0]['LoginType']],
                        'user_roleid' => $datas[0]['LoginType'],
                        'user_name' => trim($login_data[0]['RetailerName']),
                        'StockistId' => trim($login_data[0]['StockistId'])
                        );
                    }
                    
                    if($login_type== 'stockist'){
                        $loginsql = "SELECT * FROM stockist_master WHERE id='".$datas[0]['id']."' ";
                        $login_query = $this->db->query($loginsql);
                        $login_data = $login_query->result_array();
                        $data = array(
                        'user_id' => trim($datas[0]['id']),
                        'user_role' => $_SERVER['LOGINROLES'][$datas[0]['LoginType']],
                        'user_roleid' => $datas[0]['LoginType'],
                        'user_name' => trim($login_data[0]['StockistName'])
                        );
                    }
                    ...
                    /***Activity Logs***/
                    save_activity_details($_SERVER['LOGINROLES'][$datas[0]['LoginType']].' LOGIN');
                    /***Activity Logs End***/
