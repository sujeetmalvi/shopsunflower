...
class Division_model extends CI_Model {
    ...
    public function get_division_name_list(){
        //$status = ($this->uri->segment(3))?$this->uri->segment(3):'0';
        $sql = "SELECT id,DivisionName FROM division_master WHERE 1 ORDER BY DivisionName ASC";
            ...
            if ($query->num_rows() > 0) {
                $datas = $query->result_array();
                    ...
                    foreach ($datas as $key => $value) {
                        ...
                        $result.='<td>
                             <a class="btn btn-xs btn-warning" href="'.site_url('division/division_edit').'/?DivisionId='.$value["id"].'" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a> <button id="delete" onclick="deleterow(\''.$value["id"].'\',\''."row".$value["id"].'\',\''.site_url('division/division_delete').'\');" class="btn-xs btn-danger" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button>
                         </td>';
                        $result.='</tr>';
                        $i++;
                    }
