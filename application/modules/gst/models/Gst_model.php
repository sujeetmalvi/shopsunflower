...
class Gst_model extends CI_Model {
	...
	$GstApply = $this->input->post('GstApply');	 
        ...
        $sql = "SELECT id FROM gst_master WHERE `GstName` = '$GstName' OR GstValue='$GstValue'  "; //" AND `bActive`='0' 
        ...
        $data_array = array('GstName' => $GstName,'GstValue'=>$GstValue,'GstType'=>$GstType,'GstApply'=>$GstApply,'CreatedDateTime'=>$datetime);
    ...
    public function get_gst_list()
	{
       $sql = "SELECT * FROM `gst_master` ";        
        ...
        if ($query) {
			...
			<td>'.$value['GstApply'].'</td>';					

                        $result.='<td>
                             <a class="btn btn-xs btn-warning" href="'.site_url('gst/gst_edit').'/?GstId='.$value["id"].'" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a> <button id="delete" onclick="deleterow(\''.$value["id"].'\',\''."row".$value["id"].'\',\''.site_url('gst/gst_delete').'\');" class="btn-xs btn-danger" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button>
                         </td>';
                        $result.='</tr>';
                        $i++;
                    }
   ...
   public function gst_update() 
	...
	$GstApply = $this->input->post('GstApply');	
        $sql = "SELECT id FROM gst_master WHERE (`GstName` = '$GstName' OR 'GstValue'='$GstValue' ) AND `id`!=$GstId"; //" AND `bActive`='0' 
        $query = $this->db->query($sql);
        if ($query) 
		{
            if ($query->num_rows() > 0) 
		 ...
		 $data_array = array('GstName' => $GstName,'GstValue'=>$GstValue,'GstType'=>$GstType,'GstApply'=>$GstApply);
    ...
    public function get_gst_name_list($GstType){
        ...
        $sql = "SELECT id,GstName,GstValue FROM gst_master WHERE 1 ORDER BY GstName ASC";
        $query = $this->db->query($sql);
