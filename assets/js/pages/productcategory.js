$(document).ready(function (e) {
  ...
  if(validate==0){
    ...
    e.preventDefault();
    $.ajax({
            url: SITE_URL+'/productcategory/productcategory_save', // Url to which the request is send
                  ...
                  if(data) {
                    try {
                      ...
                      if(response.status == '1')
                      {
                          ...
                          window.location.href=SITE_URL+'/productcategory';
                       }
                      else
                      {   
                          //$('#submit').removeAttr('disabled');
...
if(validate==0){
    ...
    e.preventDefault();
    $.ajax({
            url: SITE_URL+'/productcategory/productcategory_update', // Url to which the request is send
            type: "POST", // Type of request to be send, called as method
            data: new FormData(this), // Data sent to server, a set of key/value pairs (i.e. form fields and values)
            contentType: false, // The content type used when sending data to the server.
            cache: false, // To unable request pages to be cached
