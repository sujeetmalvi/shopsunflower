...
$(document).ready(function (e) {
    $("#form_stockist_save").on('submit', (function (e) {
    var validate=0;

    var StockistName = $('#StockistName').val();
    if ($.trim(StockistName)=='' && validate==0) {
    	$('#StockistName').focus();
        alertify.alert('Please enter Stockist Name.');
        var validate=1;
    }
    if (!valid_alpha.test(StockistName)  && validate==0) {
    	$('#StockistName').focus();
        alertify.alert('Please enter Stockist Name in Words.');
        var validate=1;
    }

    
    var StockistContactNo = $('#StockistContactNo').val();
    if ($.trim(StockistContactNo)=='' && validate==0) {
    	$('#StockistContactNo').focus();
        alertify.alert('Please enter Stockist Contact No.');
        var validate=1;
    }
    if (!valid_phone_numeric.test(StockistContactNo)  && validate==0) {
    	$('#StockistContactNo').focus();
        alertify.alert('Please enter Stockist Contact No in Numbers between 6 to 10 digit.');
        var validate=1;
    }    

    var StockistEmail = $('#StockistEmail').val();
    if ($.trim(StockistEmail)!='' && validate==0) {
        if (!valid_emailid.test(StockistEmail)  && validate==0) {
	    	$('#StockistEmail').focus();
	        alertify.alert('Please enter Stockist Email properly.');
	        var validate=1;
    	}
    }
    ...
    if ($.trim(Address)=='' && validate==0) {
    	$('#Address').focus();
        alertify.alert('Please enter Stockist Address');
        var validate=1;
    }
...
if(validate==0){
    ...
    e.preventDefault();
    $.ajax({
            url: SITE_URL+'/stockist/stockist_save', // Url to which the request is send
                  ...
                  if(data) {
                    try {
                      ...
                      if(response.status == '1')
                      {
                          ...
                          window.location.href=SITE_URL+'/stockist/stockist_list';
                       }
                      else
                      {   
                          //$('#submit').removeAttr('disabled');
...
$(document).ready(function (e) {
    $("#form_stockist_update").on('submit', (function (e) {
    var validate=0;
...
if(validate==0){
    ...
    e.preventDefault();
    $.ajax({
            url: SITE_URL+'/stockist/stockist_update', // Url to which the request is send
            type: "POST", // Type of request to be send, called as method
            data: new FormData(this), // Data sent to server, a set of key/value pairs (i.e. form fields and values)
            contentType: false, // The content type used when sending data to the server.
            cache: false, // To unable request pages to be cached
...
if(validate==0){
    ...
    e.preventDefault();
    $.ajax({
            url: SITE_URL+'/stockist/forget_password_email_check', // Url to which the request is send
            type: "POST", // Type of request to be send, called as method
            data: new FormData(this), // Data sent to server, a set of key/value pairs (i.e. form fields and values)
            contentType: false, // The content type used when sending data to the server.
            cache: false, // To unable request pages to be cached
...
if(validate==0){
    ...
    e.preventDefault();
    $.ajax({
            url: SITE_URL+'/stockist/change_password', // Url to which the request is send
            type: "POST", // Type of request to be send, called as method
            data: new FormData(this), // Data sent to server, a set of key/value pairs (i.e. form fields and values)
            contentType: false, // The content type used when sending data to the server.
            cache: false, // To unable request pages to be cached
