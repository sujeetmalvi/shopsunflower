...
result='';
    $.post(SITE_URL + '/product/get_product_details_by_stockid', {
			...
			var PurchaseRate=0;
                    ...
                    var productGST = parseFloat(data.GstApply);


                      var PurchaseRate = actual_PurchaseRate;                    
			...
			result='';
                      ...
                      result+='<div class="td">'+data.ProductDescription+'</div>';
                      //result+='<div class="td">'+data.ShipperPack+'</div>';
                      result+='<div class="td">'+data.SalesPack+'</div>';
                      result+='<div class="td">'+data.Batch+'</div>';
                      ...
                      result+='<input type="button" class="btn btn-xs btn-danger" title="Delete" onclick="deleteme('+data.id+',\''+rowid+'\')" value="X">';
			 ...
			 result+='<input type="hidden" name="Expiry[]" class="Expiry" id="Expiry'+rowid+'" value="'+data.Expiry+'">';
			  ...
			  result+='<input type="hidden" name="PTRMargin[]" class="PTRMargin" id="PTRMargin'+rowid+'" value="'+data.PTRMargin+'">';
			  result+='<input type="hidden" name="PTSMargin[]" class="PTSMargin" id="PTSMargin'+rowid+'" value="'+data.PTSMargin+'">';
		...
		$('#productlist').html('');
		//$('#productbatchdetails').html('');
                    result+='<div id="insertbefore"></div>';
                    $('#productlist').html(result);
                   //$('#productbatchdetails').html(result);
                     $('#showtotalamount').html('0.00');
...
function get_product_details_by_id_supply(ProductId,Type){
  var result="";
  var PurchaseRate=0;
  var rowid = parseInt($('#rowid').val());
  var UserRole = $('#UserRole').val();
	$('#LoadingModal').modal('show');
    $.post(SITE_URL + '/product/get_product_details_by_id_supply', {
            ProductId:ProductId, Type:Type
        }, function(responsedata,status){
            responsedata = responsedata.trim();
            var response = JSON.parse(responsedata);
                  ...
                  //  alert(data);
                    var actual_PurchaseRate = parseFloat(data.Diprice);
                      ...
                      result+='<div class="tr" id="row'+rowid+'">';                      
                       ...
                       result+='<input type="checkbox" name="checkme" class="btn btn-xs btn-danger" value="'+data.stockid+'">';
                       }
                       else{
                       result+='';
                       }
                      ...
                      result+='<div class="td">'+data.Batch+'</div>'; 
                      result+='<div class="td">'+PurchaseRate+'</div>';                     
                      result+='<div class="td">'+data.TotalQuantity+'</div>'; 
                      //result+='<div class="td">'+data.Division+'</div>';                      
                      //result+='<div class="td">'+data.ProductDescription+'</div>';
                      //result+='<div class="td">'+data.ShipperPack+'</div>';
                      //result+='<div class="td">'+data.SalesPack+'</div>';                     
...
// }
function deleteme(id, thisrowid){
calculate_amount_onchange();
...
if(validate==0){
    ...
    $.ajax({
     ...
     function add_product_to_the_list(){
     var productid=[];
...
/*
     function add_product_to_the_list1(){
     var productid=[];
       $.each($("input[name='checkme']:checked"), function(){            
